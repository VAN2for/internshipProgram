...
COMMAND_D.registerCmd({
    ...
    main : function(socket, cookie, paras) {
        ...
        if (G.config.project.isNewPS) {
            content = fs.readFileSync(path.join(G.editorRoot, 'Template/PreviewGameWebGLPS.templet.html'), 'utf8');
        } else {
            content = fs.readFileSync(path.join(G.editorRoot, 'Template/PreviewGameWebGL.templet.html'), 'utf8');
        }
        // 替换插件文件
        ...
        fs.writeFileSync(G.gameRoot + 'PreviewGameWebGL.html', USER_SCRIPTS_D.genTemplateContent(content));
