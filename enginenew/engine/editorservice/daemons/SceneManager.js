...
function saveScene(scenePath, data)
{
    ...
    fs.writeFileSync(fullPath, JSON.stringify(data)); // 存储为json时,state文件的体积增大10倍，为了减少上传新版ps时包的体积,不用json,转成字符串

    // 拷贝文件到临时场景
    fs.copySync(fullPath, G.gameRoot + 'Temp/scene_editor.state');
    fs.copySync(fullPath, G.gameRoot + 'Temp/scene_editor_tech.state');
    // 进行备份
    console.time('backupScene')
    try {
        backupScene(scenePath + '.state');
    }
    catch(e) {
    }
    console.timeEnd('backupScene')

    // 重新打包下资源
    console.time('exploreDaemon')
    WATCH_D.exploreDaemon(G.gameRoot + 'resource/'); // 只扫描resource，避免影响效率
    console.timeEnd('exploreDaemon')
...
function backupScene(scenePath) {
    ...
    // 更新场景的版本号到project.setting
    G.config.project.sceneVersion = ext;
    AUTO_CONFIG_PROJECT_D.writeProjectSetting();
