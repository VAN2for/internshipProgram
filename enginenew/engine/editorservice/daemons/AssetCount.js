...
{
    try {
        if (!fs.existsSync(path.join(G.gameRoot, 'resource/scene')))
            return;
        ...
        searchSceneFile('resource/scene', fileList);
...
function _genTemplateContent(eventInfo)
{
    ...
    if (!Object.keys(assetCountMap).length) {
        ...
        eventInfo.content = eventInfo.content.replace(/__ASSET___PUBLISH/g, saveToPath);
    }
}
