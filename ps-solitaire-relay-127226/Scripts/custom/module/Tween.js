...
(function (ps) {
    ...
    var Tween = /** @class */ (function () {
         ...
         * @param target 目标对象。如传入组件，则目标对象为组件的gameObject
         */
        Tween.clearAll = function (target, isBehaviour) {
            if (isBehaviour === void 0) { isBehaviour = false; }
            if (!isBehaviour)
         ...
         * @param delay 延迟执行时间
         */
        Tween.shakeRotation = function (target, times, value, duration, delay) {
            if (times === void 0) { times = 4; }
            ...
            var direction = -1;
            var tweener = Tween.to(target, { rotation: ps.Mathf.angleToRadian(start + value) }, duration, easing, delay, false);
            for (var i = 0; i < times - 2; i++) {
                tweener.to({ rotation: ps.Mathf.angleToRadian(start + value * direction) }, duration * 2, easing);
                direction *= -1;
            }
            tweener.to({ rotation: ps.Mathf.angleToRadian(start) }, duration, easing).start();
            return tweener.start();
        };
        return Tween;
    }());
    ps.Tween = Tween;
    function initTarget(target, props) {
