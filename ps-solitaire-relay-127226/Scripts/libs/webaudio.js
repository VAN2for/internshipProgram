...
jWebAudio.SoundEngine.prototype = {
    ...
    addSoundSource: function (options) {
        ...
        if (typeof options.url === 'string') {
            if (window.hasBase64 && window.hasBase64() && window.getAssestByUrl) {
                var base64 = window.getAssestByUrl(options.url);
                options.data = base64;
            } else if (window.assetsPackage) {
...
jWebAudio.SoundSource.prototype.load = function (callback) {
    ...
    if (!this.isLoaded) {
        ...
        if (this.options.data) {
            ...
            var buf = basedecode2(this.options.data);
            var self = this;
            jWebAudio.context.decodeAudioData(
                buf,
                function (buffer) {
                    self.sound =new jWebAudio.WebAudioSound(buffer, self.finish);
                    // if (self.multishot) {
...
var basedecode2 = function (base64) {
...
//     div.innerHTML += "_" + $log.toString();
// }

function checkIOSAbove13() {
    // 这是对ua进行全部小写化
    var ua = navigator.userAgent.toLowerCase();
    // 判断是否IOS
    if (
        !(/\(i[^;]+;( U;)? CPU.+Mac OS X/i.test(ua))
    ...
    if (!verText) {
        ...
        verText = ua.match(/cpu\sos\s([\d.]+)/);
    }
    if (verText == null || verText[1] == null) {
        return false;
    }
    var version = Number(verText[1].indexOf('_') !== -1 ? verText[1].split('_')[0] : verText[1]);
    return version >= 14;
}
...
function playEnterSound(url, withPlay, loadAutoPlay) {
    ...
    //下面soundIsMute这个属性提前赋值，否则ios>13的版本回被下面的判断return掉，此时音效播放时，soundIsMute判断有问题

    //soundIsMute 是全局的 如果为false, playable直接播放背景音乐 否则点击后将这个字段
    //设置为fale并且播放背景音乐   另外音效的播放也要遵守这个规律来进行判断
    soundIsMute = !playsound;

    // showLog(`ios版本情况：${checkIOSAbove13()},${navigator.userAgent.toLowerCase()}`);
    if (checkIOSAbove13()) {
        if (withPlay != false) {
            playSoundEff(url, -1);
        } else {
            var key = "touchend";
            var key2 = "mouseup";
            if (window.OMG || (window.MW_CONFIG && window.MW_CONFIG.channel == "applovin")) {
                key = "touchstart";
                key2 = "mousedown";
            }

            function onStartPlaySound() {
                document.removeEventListener(key, onStartPlaySound, true);
                document.removeEventListener(key2, onStartPlaySound, true);
                console.log("ios 点击了 播放音乐");
                soundIsMute = false;//与 loadBgComplete 一样，把这个值打开，播放音乐和音效，否则点击了也不播放
                playSoundEff(url, -1);
            };

            document.addEventListener(key, onStartPlaySound, true);
            document.addEventListener(key2, onStartPlaySound, true);
        }
        return;
    }
...
function activeSound() {
    ...
    if (window.MW_CONFIG && window.MW_CONFIG.end_screen_info && window.MW_CONFIG.end_screen_info.ad_type == 288) {
        ...
        if (jWebAudio.context.sampleRate < 400000) {
            ...
            document.addEventListener(key, createBuffer, true);
            document.addEventListener(key2, createBuffer, true);
...
function playSoundEff(url, playtime, uuid) {
    var isNotCheckForcedPause = window.MW_CONFIG && window.MW_CONFIG.channel == "ironsource";
    if (window.playsound === false || soundIsMute || window.soundIsPause || (!isNotCheckForcedPause && window.forcedPause)) {
        return;
    }
    ...
    // 兼容playSoundEff没有加载完的音效的情况
    if (window[urlKey] == null || window[urlKey].sound == null) {
        window[urlKey] = window["mvSoundMgr"].addSoundSource({
            ...
            'callback': function () {
                ...
                if (window.main && window.main.gameEvent && window.ps && window.ps.AudioEvent) {
                    window.main.gameEvent.dispatch(window.ps.AudioEvent.AUDIO_PLAY_START, uuid);
                }
            },
            'finish': function () {
                if (window.main && window.main.gameEvent && window.ps && window.ps.AudioEvent) {
                    window.main.gameEvent.dispatch(window.ps.AudioEvent.AUDIO_PLAY_COMPLETE, uuid);
                }
            }
        });
...
function loadBgComplete() {
    ...
    if (soundIsMute) //DSP的静音模式
    {
        ...
        if (window.OMG) {
            ...
            if (window.MW_CONFIG && window.MW_CONFIG.channel == "applovin") {
                key = "touchstart";
                key2 = "mousedown";
            }
        ...
        function onStartPlaySound() {
            document.removeEventListener(key, onStartPlaySound, true);
            document.removeEventListener(key2, onStartPlaySound, true);
            console.log("bg加载完成，点击了 播放音乐");
            soundIsMute = false;
            if (window["bg_mp3"]) {
                window["bg_mp3"].sound.play();
                if (soundIsPause || forcedPause) {
                    window["bg_mp3"].sound.setVolume(0);
                }
            }
        };
        ...
        // 对外PS在编辑器中设置非自动播放
        if (!window.ps || (window.ps && window.ps.cfg.AUTO_PLAY_BGM)) {
...
function registerListener() {
    ...
    if ("ad-event-resume" in window && "ad-event-pause" in window) {
        window.addEventListener("ad-event-resume", onFocusHandler, false);
        window.addEventListener("ad-event-pause", onBlurHandler, false);
    }
...
function setVolume(url, volume) {
    ...
    if (window[urlKey].sound) {
        window[urlKey].sound.setVolume(volume)
    }
}