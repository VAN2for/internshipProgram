namespace ps {
    ...
    export class AStarMap {
        ...
        data: number[][] = [];
        constructor(rows: number, cols: number, v: 0 | 1 = 0) {
            this.rows = rows;
            this.cols = cols;
            let arr = [];
            for (let i = 0; i < cols; i++)  arr.push(v);
            for (let i = 0; i < rows; i++)  this.data[i] = arr.concat();
        }

        /** 设置障碍或通路 */
        set(x: number, y: number, v: number) {
            if (this.data[x]) this.data[x][y] = v;
        }
        get(x: number, y: number) {
            if (this.data[x]) return this.data[x][y];
        }
        /** 设置整张地图 */
        setAll(v: 0 | 1) {
        ...
        /** 设置一片矩形区域 */
        setArea(x: number, y: number, w: number, h: number, v: 0 | 1) {
            for (let i = x; i < x + w; i++) {
                for (let j = y; j < y + h; j++) {
                    this.set(i, j, v);
                }
        ...
        /** 寻路 */
        searchRoad(startX: number, startY: number, endX: number, endY: number) {
...
namespace astar {
    //其中的map.data是二维数组
    export function getPath(map, start_x, start_y, end_x, end_y) {
        var openList = [],    //开启列表
            closeList = [],   //关闭列表
            result = [],      //结果数组
