/**
 * 寻找物品（触控）可操作区域 - 编辑器菜单拓展
 * @author JingBin
 * @argument 第一个参数为自定义的脚本组件类名
 * @argument 第二个参数为重载的绘制函数
 */
G.extend.inspector('ps.FindItemTouchOptionAreaView', function () {
    var self = this,
        target = self.target;
    ...
    gui.line([
        ...
        gui.text('寻找物品（触控）可操作区域', { align: 'center' }),
        gui.divider({ color: "#333333" })
    ]);
    ...
    gui.columnWidths = ['120+0.2', '60+0.1'];
    gui.line([
        gui.text('触控到错误区域后的音效'),
        gui.stringInput({ bind: '_soundName' }),
    ]);

    gui.columnWidths = ['120+0.2', '60+0.1'];
    gui.line([
        gui.text('触控到错误区域后的埋点'),
