/**
 * 多视频列表管理器组件，新版PS工具使用 - 编辑器菜单拓展
 * @author JingBin
 * @argument 第一个参数为自定义的脚本组件类名
 * @argument 第二个参数为重载的绘制函数
 */
G.extend.inspector('ps.VideosManager', function () {
    var self = this,
        target = self.target;
    ...
    gui.line([
        ...
        gui.text('多视频列表管理器组件，新版PS工具使用', { align: 'center' }),
        gui.divider({ color: "#333333" })
    ]);
    ...
    // if (target.playType == void 0) target.playType = VideoPlayType.TOUCH_PLAY;
    // gui.line([
    //     gui.text('播放类型'),
