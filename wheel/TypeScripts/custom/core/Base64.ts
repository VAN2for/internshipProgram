...
let _hasBase64 = void 0
/**
 * 有base64资源
 * @emphasis 改函数多处用到，无特殊需求勿改名
 */
function hasBase64() {
    if (_hasBase64 == void 0) _hasBase64 = !ps.Tools.objIsNull(assetsBase64())
    return _hasBase64
}

let dco: any = 0
let dcoConfig = void 0
let _getDcoConfiged = void 0
...
function getDcoConfig() {
    if (_getDcoConfiged) return
    _getDcoConfiged = true
    const value = getQueryString("dco_gid")
    if (dco != value) { dco = value; }
    if (window["HttpAPI"] && !isSendDone) {
        isSendDone = true;
        window["HttpAPI"].sendPoint("dco&cls=" + dco);
    }
    if (hasBase64()) dcoConfig = ps.Tools.strToJson(assetsBase64()["dco_config_json"])
}

let _hasDcoConfig = void 0
/**
 * 有DCO配置
 */
...
let dcoData = void 0
let _getDcoDataed = void 0
/**
 * 获取DCO数据
 */
function getDcoData() {
    if (_getDcoDataed) return
    _getDcoDataed = true
    if (hasDcoConfig()) {
        dcoData = dcoConfig[dco]
        if (ps.Tools.objIsNull(dcoData)) dcoData = dcoConfig[Object.keys(dcoConfig)[0]] //默认取配置中第一个
    }
}

let _hasDcoData = void 0
/**
 * 有DCO数据
 */
function hasDcoData() {
...
let dcoRes = void 0
let _getDcoResed = void 0
/**
 * 获取DCO资源
 */
function getDcoRes() {
    if (_getDcoResed) return
    _getDcoResed = true
    if (hasDcoData()) dcoRes = dcoData["res"]
}

let _hasDcoRes = void 0
/**
 * 有DCO资源
 */
function hasDcoRes() {
 ...
 */
function getDcoKey(key: string) {
    let dcoKey = key
    if (hasDcoRes() && dcoRes[key]) dcoKey = dcoRes[key]
    return dcoKey
}
 ...
 * @emphasis 改函数多处用到，无特殊需求勿改名
 */
function getAssestByKey(key: string) {
    if (hasBase64()) {
        key = getDcoKey(key);
 ...
 * @emphasis 改函数多处用到，无特殊需求勿改名
 */
function getAssestByUrl(url: string): string {
    if (hasBase64()) {
        const arr = url.split("\/");
        url = arr[arr.length - 1];
        const key = url.replace(".", "_");
