...
function ExchangeLangFile(fullPath, rootPath) {
    var fs = require('fs-extra');
    var settingPath = path.join(G.editorRoot, 'ExcelToConfig.setting');
    if (!fs.existsSync(settingPath)) return;
    var config = fs.readJsonSync(settingPath, { throws : false });
    if(config == null || config.lang == null) {
        console.log("ExchangeLangFile faild! can not find tool path.", fullPath);
        return;
    }
    var tool = require(path.join(config.lang.toolPath, "dist.js"));
    const languagesJson = tool.getLanguagesJson(fullPath)
    fs.writeFileSync(path.join(rootPath, config.lang.outputPath), JSON.stringify(languagesJson, undefined, 4));
    COMMAND_D.broadcast('LANG_UPDATE');
}

function ExchangeGameConfigFile(fullPath, rootPath) {
    var fs = require('fs-extra');
    var settingPath = path.join(G.editorRoot, 'ExcelToConfig.setting');
    if (!fs.existsSync(settingPath)) return;
    var config = fs.readJsonSync(settingPath, { throws : false });
    if(config == null || config.gameConfig == null) {
        console.log("ExchangeGameConfigFile faild! can not find tool path.", fullPath);
        return;
    }
    var tool = require(path.join(config.gameConfig.toolPath, "index.js"));
    const jsonConfig = tool.getGameConfigJson(fullPath);
    const tsConfig = tool.getGameConfigTypeScript(jsonConfig);
    fs.writeFileSync(path.join(rootPath, config.gameConfig.outputJsonPath), JSON.stringify(jsonConfig, undefined, 4));
    fs.writeFileSync(path.join(rootPath, config.gameConfig.outputTsPath), tsConfig);
    COMMAND_D.broadcast('GAMECONFIG_UPDATE');
}
...
var _dealWithFileEvent = function(event, rootPath, fileName, virtualDir) {
    ...
    if (fileName && FS_EXPAND_D.isMeta(fileName)) {
        ...
        if (fsExtra.existsSync(fullPath) && rootPath == G.gameRoot) {
            if (fileName == "lang.xlsx") {
                return ExchangeLangFile(fullPath, rootPath);
            } else if (fileName == "gameConfig.xlsx") {
                return ExchangeGameConfigFile(fullPath, rootPath);
            }
        }
